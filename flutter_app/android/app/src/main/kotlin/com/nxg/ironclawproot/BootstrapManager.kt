...
class BootstrapManager(
    ...
    fun setupDirectories() {
        listOf(rootfsDir, tmpDir, homeDir, configDir, "$homeDir/.IronClaw", libDir).forEach {
            File(it).mkdirs()
        }
    ...
    private fun configureRootfs() {
        ...
        aptConfDir.mkdirs()
        File(aptConfDir, "01-IronClaw-proot").writeText(
        ...
        dpkgConfDir.mkdirs()
        File(dpkgConfDir, "01-IronClaw-proot").writeText(
            "force-unsafe-io\n" +
            "no-debsig\n" +
            "force-overwrite\n" +
            "force-depends\n" +
        ...
        listOf(
            ...
            // IronClaw runtime directories (can't mkdir at runtime)
            "$rootfsDir/root/.IronClaw",
            "$rootfsDir/root/.IronClaw/data",
            "$rootfsDir/root/.IronClaw/memory",
            "$rootfsDir/root/.IronClaw/skills",
            "$rootfsDir/root/.IronClaw/config",
            "$rootfsDir/root/.IronClaw/extensions",
            "$rootfsDir/root/.IronClaw/logs",
            "$rootfsDir/root/.config/IronClaw",
            "$rootfsDir/root/.local/share",
            "$rootfsDir/root/.cache",
            "$rootfsDir/root/.cache/IronClaw",
    ...
    fun createBinWrappers(packageName: String) {
        ...
        // "bin": "cli.js"  OR  "bin": {"IronClaw": "bin/IronClaw.js", ...}
    ...
    fun installBionicBypass() {
        val bypassDir = File("$rootfsDir/root/.IronClaw")
        if (!bypassDir.exists()) {
            bypassDir.mkdirs()
        }
        ...
        val cwdFixContent = """
// IronClaw CWD Fix - Auto-generated
...
""".trimIndent()
        ...
        //    Usage: node /root/.IronClaw/node-wrapper.js <script> [args...]
        val wrapperContent = """
// IronClaw Node Wrapper - Auto-generated
...
require('/root/.IronClaw/proot-compat.js');
...
// IronClaw Proot Compatibility Layer - Auto-generated
...
""".trimIndent()
        ...
        // 4. Bionic bypass — comprehensive runtime patcher for IronClaw.
        //    Loaded via NODE_OPTIONS="--require /root/.IronClaw/bionic-bypass.js"
        val bypassContent = """
// IronClaw Bionic Bypass - Auto-generated
...
""".trimIndent()
        ...
        val exportLine = "export NODE_OPTIONS=\"--require /root/.IronClaw/bionic-bypass.js\""

        val existing = if (bashrc.exists()) bashrc.readText() else ""
        if (!existing.contains("bionic-bypass")) {
            bashrc.appendText("\n# IronClaw Bionic Bypass\n$exportLine\n")
        }

        // Pre-seed IronClaw.json with gateway.mode=local so the gateway
        // doesn't reject startup with "set gateway.mode=local" (#93, #90).
        val configFile = File(bypassDir, "IronClaw.json")
...
""".trimIndent())
    ...
    /** Read a file from inside the rootfs (e.g. /root/.IronClaw/IronClaw.json). */
